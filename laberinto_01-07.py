...
class Laberinto():
    def  __init__(self,_algoritmo):
        ...
        #si no lo es, guardamos False
            ...
            print(self.paredes)                       #ya es una matriz
    ...
    def resolver(self):
        ...
        #------------------------------------------------------------------------
        nodo_inicial=Nodo(self.inicio,None) #paso 1
        frontera.agregar_nodo(nodo_inicial) #paso 1
        self.nodos_explorados=[]            #paso 2
        while True:                         #paso 3
            if frontera.esta_vacia():
                raise Exception("No hay solucion") #se puede hacer un return "no hay solucion" no imprime; raise si imprime
            nodo_actual=frontera.quitar_nodo()
            if nodo_actual.estado==self.meta: 
                print("Encontre la meta")
                return
            self.nodos_explorados.append(nodo_actual)     #paso 3.4
            vecinos=self.expandir_nodos(nodo_actual) #paso 3.5
